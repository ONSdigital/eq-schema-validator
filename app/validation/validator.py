...
class Validator:  # pylint: disable=too-many-lines
    ...
    def _validate_blocks(  # noqa: C901 pylint: disable=too-many-branches
        ...
        for block in group.get('blocks'):
            ...
            if block['type'] == 'CalculatedSummary':
                ...
                errors.extend(
                    self._validate_relationship_collector_contains_single_relationship_answer(
                        block
                    )
                )
    ...
    def _validate_json_against_schema(self, json_to_validate):
        try:
            base_uri = pathlib.Path(
                os.path.abspath('schemas/questionnaire_v1.json')
            ).as_uri()
    ...
    def _validate_when_rule(self, when_clause, answer_ids_with_group_id, referenced_id):
        ...
        for when in when_clause:
            ...
            if 'comparison_id' in when:
                errors.extend(
                    self._validate_comparison_id_in_when_rule(
                        when, answer_ids_with_group_id, referenced_id
                    )
                )
                ...
                errors.append(
                    self._error_message(
                        'The answer id - {} in the {} key of the "when" clause for {} does not exist'.format(
                            when[id_ref_key], id_ref_key, referenced_id
                        )
                    )
                )
    ...
    def _validate_comparison_id_in_when_rule(
        self, when, answer_ids_with_group_id, referenced_id
    ):
        """
        ...
        comparison_answer_type = answer_ids_with_group_id[when['comparison_id']][
            'answer'
        ]['type']
        ...
        if condition in conditions_requiring_list_match_values:
            ...
            errors.append(
                self._error_message(
                    f'The answers used as comparison_id `{comparison_id}` and answer_id `{answer_id}` in the `when` '
    ...
    def _validate_placeholders(
        ...
        for string_with_placeholders in strings_with_placeholders or []:
            ...
            for placeholder_definition in string_with_placeholders.get('placeholders'):
                placeholder_definition_names.append(
                    placeholder_definition['placeholder']
                )

                transforms = placeholder_definition.get('transforms')
                answer_ids_to_validate, metadata_ids_to_validate = self._get_placeholder_source_ids(
                    placeholder_definition, transforms
                )
                ...
                if transforms:
                    errors.extend(
                        self._validate_placeholder_transforms(
                            transforms, block_json['id']
                        )
                    )
    ...
    def _validate_relationship_collector_contains_single_relationship_answer(
        self, block
    ):
    ...
    def _validate_relationship_collector_answer_type(self, block):
        ...
        return (
            ...
            if block['question']['answers'][0]['type'] != 'Relationship'
            else []
        )
